...
def callback_pointer(p):
    ...
    if counter % 4 == 1:
        if (a==a_after):
            counter_second += 1
            if counter_second >=3:
                counter_second = 0 #120120120 sequence
    '''if counter % 4 == 0:
                    #command
                if counter % 4 == 2:
                    #command
...
Button_B = machine.Pin(16, machine.Pin.IN,machine.Pin.PULL_UP)        # Button B has a 100K pullup
...
Button_B.irq(trigger= machine.Pin.IRQ_RISING, handler=callback_change_time)
oled.fill(1)
oled.show()
time.sleep(1)
